...
Here, [Iacono et al. (2019)](https://doi.org/10.1186/s13059-019-1713-4) reported a novel way to build the co-expression matrix and apply graph-theory metrics to single-cell papers to identify GRN with high accuracy.
...
Details in bigScale installation and tutorial is posted in [github](https://github.com/dfajar2/bigSCale).
...
In brief, the author perform DE analysis between all pairs of clusters they identified, and translate the probability of DE of a gene to a z-score.
Then they compute gene-gene correlation based on the z-score. 
In their report, using the z-scores circumvent the issues of drop-outs and imputation is no longer needed.

Next they retain genes with significant correlation to build the GRN, that genes are nodes and correlation are the weighted edge. 
Finally, they use GO to refine the network, only keeping biologically functional correlations.
...
The foremost task of such analysis was to quantify the gene's importance in a network. To do so, the authors explore several measures of node centrality.
...
Hopefully, I will later have some data that I can apply this awesome techniques in.
