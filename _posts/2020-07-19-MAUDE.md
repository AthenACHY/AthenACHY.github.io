...
Using MAUDE
======
Pooled CRISPR library screen has become a very popular tool for forward genetic screens that study transcriptional regulation, enhancer elements and pathway components, etc.
Although, CRISPR-i is ways more effective than sgRNA inhibition, CRISPR efficiency is highly dependent on its sgRNA, both on its activity and specificity. 
...
<figure>
<p align="left">
<img src="/img/posts/2020_07_19_MAUDE/2020_bin_dist.png" width="300" height="300" title="Distributions in bins">
</p>
</figure>
...
By testing the results of gRNA effect using only the high and the low bins compared to using all four bins for the learning, we can see that Z score learnt from only using the low and high bins are in general over-estimated; and inaccuracy are particularly high when Z score get larger than 3. 

<figure>
<p align="left">
<img src="/img/posts/2020_07_19_MAUDE/density_plot" width="350" height="800" title="Distributions in bins">
</p>
</figure>

More interesting, Z score cannot be centered if using only two bins of the data (even with 4478 non-targeting control sgRNA).
It may be due to the fact that the bin-fractions of the two bins (low=25%, high=2.8%) are not big enough for accurate normalisation.

Still, it will be good to see that there will be a further development that enable us to have better inference with fewer bins, as most labs typically can afford to sequence the unsorted background vs the selected bin.